...
When a skill mentions a role (e.g. "set the AFK-ready status"), write the corresponding string as the file's status. **How** it's stored depends on the file: wayfinder-pipeline files (`map.md`, `spec.md`, `decisions/`, `tickets/`) carry it in YAML frontmatter as `status:` (see `issue-tracker.md`); triage request files carry it as a `**Status:** <string>` line near the top.
