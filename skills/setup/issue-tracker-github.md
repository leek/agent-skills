...
The pipeline nests three levels, named for their agile equivalents. Each is a GitHub issue:

| Agile name | Issue | Made by | Holds |
|---|---|---|---|
| **epic** | labelled `wayfinder:map` | `wayfinder` | many features |
| **feature** | the spec (PRD) body | `to-spec` | many stories |
...
Create a GitHub issue at the level above, as a sub-issue of its parent when it has one.
...
- **Map**: one issue labelled `wayfinder:map` holding the Destination / Notes / Decisions-so-far / Handed-off / fog body
...
- **Hand off**: `to-spec` creates the feature issue, attaches it as a sub-issue of the map, and appends one line to the map's Handed off section. The map stays open — close it only after its last feature is handed off
- **Status**: `gh api repos/{owner}/{repo}/issues/{feature}/sub_issues` per handed-off feature, counted by state; where that endpoint is unavailable, fall back to the `Parent: #<n>` body line. The map never stores build state
