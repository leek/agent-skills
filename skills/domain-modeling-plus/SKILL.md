---
name: domain-modeling-plus
description: Build and sharpen a project's domain model — challenge terms against the glossary, resolve fuzzy language via AskUserQuestion, stress-test concepts with concrete scenarios, and record CONTEXT.md entries and ADRs the moment decisions land. Use when pinning down domain terminology or a ubiquitous language, recording an architectural decision, or when another skill (grill-me-plus, wayfinder-plus, to-spec-plus) needs the domain model maintained.
---
...
When the user uses vague or overloaded terms, resolve them through `AskUserQuestion` (Claude Code) or `request_user_input` (Codex): the candidate canonical terms as options, your recommendation first, each option's description saying what choosing it commits the model to. "You're saying 'account' — is that the Customer or the User?" is a click-to-answer decision, not an open-ended essay prompt. Fall back to asking in chat when no structured tool is available.
