...
- Dispatch through whatever sub-agent the harness provides, picking a **read-only** one for source sweeps (it returns findings as text and the dispatcher saves the file) and a **general-purpose** one when the research needs web fetches, other tool calls, or must write the findings file itself. In Claude Code that is the Agent tool with `Explore` and `general-purpose` respectively. Hand the sub-agent the question plus **The research brief** below.
