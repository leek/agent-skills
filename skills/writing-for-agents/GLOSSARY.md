...
The domain model for writing any document an agent consumes. Such a document exists to wrangle determinism out of a stochastic system; the root virtue is **Predictability**, and every term below is a lever on it. This is the disclosed reference for [`writing-for-agents`](SKILL.md).
...
_Failure mode when unearned._ A passage that restates the **environment** (`composer.json` / `package.json` scripts, config files, the directory layout, `--help` output) rather than something only the writer knows. The environment is itself a source of truth, so the copy earns its load only when the lookup it replaces is expensive; a one-file, one-command lookup left to the environment cannot go stale, while the cache can. Cache what the agent cannot find by looking: the unwritten convention, the reason behind a choice, the gotcha no config confesses.

_Avoid_: mirroring, restating config, documenting the obvious file
