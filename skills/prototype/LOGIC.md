...
When the model is being prototyped for a Laravel codebase, write the module in plain JS anyway; it is a sketch of the transitions, not the eventual PHP. The value transferred is the state set, the legal transitions, and the field shape; the language it lands in is a separate decision.
...
Once the prototype has answered its question, capture the answer, then capture the prototype the way the [SKILL](SKILL.md) describes. The logic-specific mapping: the validated reducer / machine / function set lifts into the real module (the decision, absorbed); the HTML file itself stays as the clearly-named throwaway that keeps the prototype as a primary source, never a git branch. Being one self-contained file, it stays trivially re-runnable wherever it sits.
