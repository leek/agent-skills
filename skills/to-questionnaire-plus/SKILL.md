---
name: to-questionnaire-plus
description: Turn a decision the user can't fully answer into a Markdown questionnaire for someone else to fill in, async or over a meeting. Use Claude Code's AskUserQuestion or Codex's request_user_input for the setup questions. Use when the user says "to questionnaire", needs answers from a stakeholder, vendor, or compliance contact, or a grilling branch is blocked on knowledge someone else holds.
---
...
**Grill the send, not the subject.** Interview the user only about the _send_, which they can always answer: who it goes to, and what they need back. The questions in the document then target the **gap** between what the recipient knows and what the user needs. Route the setup questions through `AskUserQuestion` (Claude Code) or `request_user_input` (Codex) when available — the full question protocol lives in `grill-me-plus`.
